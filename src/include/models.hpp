...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(46, 44, "end", "model_lmm");
    return reader;
}

class model_lmm : public prob_grad {
private:
    int n_sp;
    int n_obs;
    vector<int> map;
...
public:
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        // initialize member variables
        try {
            context__.validate_dims("data initialization", "n_sp", "int", context__.to_vec());
            n_sp = int(0);
            vals_i__ = context__.vals_i("n_sp");
            pos__ = 0;
            n_sp = vals_i__[pos__++];
            context__.validate_dims("data initialization", "n_obs", "int", context__.to_vec());
            n_obs = int(0);
            ...
            pos__ = 0;
            n_obs = vals_i__[pos__++];
            validate_non_negative_index("map", "n_obs", n_obs);
            context__.validate_dims("data initialization", "map", "int", context__.to_vec(n_obs));
            validate_non_negative_index("map", "n_obs", n_obs);
            map = std::vector<int>(n_obs,int(0));
            vals_i__ = context__.vals_i("map");
            pos__ = 0;
            size_t map_limit_0__ = n_obs;
            for (size_t i_0__ = 0; i_0__ < map_limit_0__; ++i_0__) {
                map[i_0__] = vals_i__[pos__++];
            }
            ...
            ++num_params_r__;
            ++num_params_r__;
            validate_non_negative_index("z_b0", "n_sp", n_sp);
            num_params_r__ += n_sp;
            validate_non_negative_index("z_b1", "n_sp", n_sp);
            num_params_r__ += n_sp;
            validate_non_negative_index("sig_b0", "n_sp", n_sp);
            num_params_r__ += n_sp;
            validate_non_negative_index("sig_b1", "n_sp", n_sp);
            num_params_r__ += n_sp;
            ++num_params_r__;
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        if (!(context__.contains_r("b0_mean")))
            throw std::runtime_error("variable b0_mean missing");
        vals_r__ = context__.vals_r("b0_mean");
        pos__ = 0U;
        context__.validate_dims("initialization", "b0_mean", "double", context__.to_vec());
        double b0_mean(0);
        b0_mean = vals_r__[pos__++];
        try {
            writer__.scalar_unconstrain(b0_mean);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable b0_mean: ") + e.what());
        }
        ...
        if (!(context__.contains_r("z_b0")))
            throw std::runtime_error("variable z_b0 missing");
        vals_r__ = context__.vals_r("z_b0");
        pos__ = 0U;
        validate_non_negative_index("z_b0", "n_sp", n_sp);
        context__.validate_dims("initialization", "z_b0", "double", context__.to_vec(n_sp));
        std::vector<double> z_b0(n_sp,double(0));
        for (int i0__ = 0U; i0__ < n_sp; ++i0__)
            z_b0[i0__] = vals_r__[pos__++];
        for (int i0__ = 0U; i0__ < n_sp; ++i0__)
            try {
            writer__.scalar_unconstrain(z_b0[i0__]);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable z_b0: ") + e.what());
        }

        if (!(context__.contains_r("z_b1")))
            throw std::runtime_error("variable z_b1 missing");
        vals_r__ = context__.vals_r("z_b1");
        pos__ = 0U;
        validate_non_negative_index("z_b1", "n_sp", n_sp);
        context__.validate_dims("initialization", "z_b1", "double", context__.to_vec(n_sp));
        std::vector<double> z_b1(n_sp,double(0));
        for (int i0__ = 0U; i0__ < n_sp; ++i0__)
            z_b1[i0__] = vals_r__[pos__++];
        for (int i0__ = 0U; i0__ < n_sp; ++i0__)
            try {
            writer__.scalar_unconstrain(z_b1[i0__]);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable z_b1: ") + e.what());
        }

        if (!(context__.contains_r("sig_b0")))
            throw std::runtime_error("variable sig_b0 missing");
        vals_r__ = context__.vals_r("sig_b0");
        pos__ = 0U;
        validate_non_negative_index("sig_b0", "n_sp", n_sp);
        context__.validate_dims("initialization", "sig_b0", "double", context__.to_vec(n_sp));
        std::vector<double> sig_b0(n_sp,double(0));
        for (int i0__ = 0U; i0__ < n_sp; ++i0__)
            sig_b0[i0__] = vals_r__[pos__++];
        for (int i0__ = 0U; i0__ < n_sp; ++i0__)
            try {
            writer__.scalar_lb_unconstrain(0,sig_b0[i0__]);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable sig_b0: ") + e.what());
        }

        if (!(context__.contains_r("sig_b1")))
            throw std::runtime_error("variable sig_b1 missing");
        vals_r__ = context__.vals_r("sig_b1");
        pos__ = 0U;
        validate_non_negative_index("sig_b1", "n_sp", n_sp);
        context__.validate_dims("initialization", "sig_b1", "double", context__.to_vec(n_sp));
        std::vector<double> sig_b1(n_sp,double(0));
        for (int i0__ = 0U; i0__ < n_sp; ++i0__)
            sig_b1[i0__] = vals_r__[pos__++];
        for (int i0__ = 0U; i0__ < n_sp; ++i0__)
            try {
            writer__.scalar_lb_unconstrain(0,sig_b1[i0__]);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable sig_b1: ") + e.what());
        }
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        try {
            ...
            if (jacobian__)
                b0_mean = in__.scalar_constrain(lp__);
            else
                b0_mean = in__.scalar_constrain();
            ...
            vector<local_scalar_t__> z_b0;
            size_t dim_z_b0_0__ = n_sp;
            z_b0.reserve(dim_z_b0_0__);
            for (size_t k_0__ = 0; k_0__ < dim_z_b0_0__; ++k_0__) {
                if (jacobian__)
                    z_b0.push_back(in__.scalar_constrain(lp__));
                else
                    z_b0.push_back(in__.scalar_constrain());
            }

            vector<local_scalar_t__> z_b1;
            size_t dim_z_b1_0__ = n_sp;
            z_b1.reserve(dim_z_b1_0__);
            for (size_t k_0__ = 0; k_0__ < dim_z_b1_0__; ++k_0__) {
                if (jacobian__)
                    z_b1.push_back(in__.scalar_constrain(lp__));
                else
                    z_b1.push_back(in__.scalar_constrain());
            }

            vector<local_scalar_t__> sig_b0;
            size_t dim_sig_b0_0__ = n_sp;
            sig_b0.reserve(dim_sig_b0_0__);
            for (size_t k_0__ = 0; k_0__ < dim_sig_b0_0__; ++k_0__) {
                if (jacobian__)
                    sig_b0.push_back(in__.scalar_lb_constrain(0,lp__));
                else
                    sig_b0.push_back(in__.scalar_lb_constrain(0));
            }

            vector<local_scalar_t__> sig_b1;
            size_t dim_sig_b1_0__ = n_sp;
            sig_b1.reserve(dim_sig_b1_0__);
            for (size_t k_0__ = 0; k_0__ < dim_sig_b1_0__; ++k_0__) {
                if (jacobian__)
                    sig_b1.push_back(in__.scalar_lb_constrain(0,lp__));
                else
                    sig_b1.push_back(in__.scalar_lb_constrain(0));
            }
            ...
            // transformed parameters
            validate_non_negative_index("b0", "n_sp", n_sp);
            vector<local_scalar_t__> b0(n_sp);
            stan::math::initialize(b0, DUMMY_VAR__);
            stan::math::fill(b0,DUMMY_VAR__);
            validate_non_negative_index("b1", "n_sp", n_sp);
            vector<local_scalar_t__> b1(n_sp);
            ...
            for (int j = 1; j <= n_sp; ++j) {
                ...
                stan::model::assign(b0, 
                            ...
                            (b0_mean + (get_base1(sig_b0,j,"sig_b0",1) * get_base1(z_b0,j,"z_b0",1))), 
                            "assigning variable b0");
                stan::model::assign(b1, 
                            stan::model::cons_list(stan::model::index_uni(j), stan::model::nil_index_list()), 
                            (b1_mean + (get_base1(sig_b1,j,"sig_b1",1) * get_base1(z_b1,j,"z_b1",1))), 
                            "assigning variable b1");
            }

            // validate transformed parameters
            for (int i0__ = 0; i0__ < n_sp; ++i0__) {
            ...
            lp_accum__.add(normal_log<propto__>(b0_mean, 0, 1));
            lp_accum__.add(normal_log<propto__>(b1_mean, 0, 1));
            lp_accum__.add(normal_log<propto__>(z_b0, 0, 1));
            lp_accum__.add(normal_log<propto__>(z_b1, 0, 1));
            for (int j = 1; j <= n_sp; ++j) {

                lp_accum__.add(normal_log<propto__>(get_base1(sig_b0,j,"sig_b0",1), 0, 1));
                if (get_base1(sig_b0,j,"sig_b0",1) < 0) lp_accum__.add(-std::numeric_limits<double>::infinity());
                else lp_accum__.add(-normal_ccdf_log(0, 0, 1));
                lp_accum__.add(normal_log<propto__>(get_base1(sig_b1,j,"sig_b1",1), 0, 1));
                if (get_base1(sig_b1,j,"sig_b1",1) < 0) lp_accum__.add(-std::numeric_limits<double>::infinity());
                else lp_accum__.add(-normal_ccdf_log(0, 0, 1));
            }
            ...
            for (int i = 1; i <= n_obs; ++i) {
                ...
                lp_accum__.add(normal_log<propto__>(get_base1(y,i,"y",1), (get_base1(b0,get_base1(map,i,"map",1),"b0",1) + (get_base1(b1,get_base1(map,i,"map",1),"b1",1) * get_base1(x,i,"x",1))), sig_res));
            }
    ...
    void get_param_names(std::vector<std::string>& names__) const {
        ...
        names__.push_back("z_b0");
        names__.push_back("z_b1");
        names__.push_back("sig_b0");
        names__.push_back("sig_b1");
    ...
    void write_array(RNG& base_rng__,
        ...
        (void) function__;  // dummy to suppress unused var warning
        // read-transform, write parameters
        double b0_mean = in__.scalar_constrain();
        ...
        vector<double> sig_b0;
        size_t dim_sig_b0_0__ = n_sp;
        for (size_t k_0__ = 0; k_0__ < dim_sig_b0_0__; ++k_0__) {
            sig_b0.push_back(in__.scalar_lb_constrain(0));
        }
        vector<double> sig_b1;
        size_t dim_sig_b1_0__ = n_sp;
        for (size_t k_0__ = 0; k_0__ < dim_sig_b1_0__; ++k_0__) {
            sig_b1.push_back(in__.scalar_lb_constrain(0));
        }
        ...
        vars__.push_back(b1_mean);
            for (int k_0__ = 0; k_0__ < n_sp; ++k_0__) {
            vars__.push_back(z_b0[k_0__]);
            }
            for (int k_0__ = 0; k_0__ < n_sp; ++k_0__) {
            vars__.push_back(z_b1[k_0__]);
            }
            for (int k_0__ = 0; k_0__ < n_sp; ++k_0__) {
            vars__.push_back(sig_b0[k_0__]);
            }
            for (int k_0__ = 0; k_0__ < n_sp; ++k_0__) {
            vars__.push_back(sig_b1[k_0__]);
            }
    ...
    void constrained_param_names(std::vector<std::string>& param_names__,
        ...
        param_names__.push_back(param_name_stream__.str());
        for (int k_0__ = 1; k_0__ <= n_sp; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "z_b0" << '.' << k_0__;
            param_names__.push_back(param_name_stream__.str());
        }
        for (int k_0__ = 1; k_0__ <= n_sp; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "z_b1" << '.' << k_0__;
            param_names__.push_back(param_name_stream__.str());
        }
        for (int k_0__ = 1; k_0__ <= n_sp; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "sig_b0" << '.' << k_0__;
            param_names__.push_back(param_name_stream__.str());
        }
        for (int k_0__ = 1; k_0__ <= n_sp; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "sig_b1" << '.' << k_0__;
            param_names__.push_back(param_name_stream__.str());
        }
        param_name_stream__.str(std::string());
